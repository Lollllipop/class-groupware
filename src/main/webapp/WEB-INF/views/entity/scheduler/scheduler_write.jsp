...
<body>
								...
								<form method="post" name="scheduler" id="target" role="form">
									<div class="form-group">
										...
										<div class="input-group date">
											<input type="text" name="scheduler_content"> <input
												type="hidden" id="class_idx" value="1" name="class_idx" />
										</div>
									</div>
									<div class="form-group">
										<label>From: </label>
									...
									<div class="form-group">
										<div class="input-group">
											...
											<button type="button" class="btn btn-warning" id="insert">추가</button>
										</div>
									</div>
								</form>
							</div>
	...
	<script type="text/javascript">
		$(function() {
			...
			var scheduler_startdate = '${scheduler_startdate}';

			if (scheduler_startdate) {

				var selectdate = new Date(scheduler_startdate);
				var tomorrow = new Date();
				tomorrow.setDate(selectdate.getDate() + 1);

				//From의 초기값을 오늘 날짜로 설정
				$('#datepicker').datepicker('setDate', selectdate); //(-1D:하루전, -1M:한달전, -1Y:일년전), (+1D:하루후, -1M:한달후, -1Y:일년후)
				//To의 초기값을 내일로 설정
				$('#datepicker2').datepicker('setDate', tomorrow); //(-1D:하루전, -1M:한달전, -1Y:일년전), (+1D:하루후, -1M:한달후, -1Y:일년후)
			...
			$('#insert').click(function() {
				...
				formObj.attr("action", "/scheduler/tch/insert");
